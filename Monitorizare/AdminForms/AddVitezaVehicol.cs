...
namespace Monitorizare.AdminForms
{
    public partial class AddVitezaVehicol : Form
    {
        ...
        private void btAdauga_Click(object sender, EventArgs e)
        {
            try
            {
                using (var connection = _connection)
                {
                    ...
                    command.CommandText = "UPDATE Vehicul SET Viteza = $Viteza" +
                        " WHERE Id = $Id";
                    command.Parameters.AddWithValue("$Id", Convert.ToInt32(tbId.Text));
                    command.Parameters.AddWithValue("$Viteza", Convert.ToDouble(tbViteza.Text));
        ...
        private void tbViteza_KeyPress(object sender, KeyPressEventArgs e)
        {
