...
{
    public partial class AddTimpSosireVehicol : Form
    {
        private string connectionString = "Data Source = Monitorizare.db";
        private readonly SqliteConnection _connection;
        public AddTimpSosireVehicol()
        {
            InitializeComponent();
        ...
        private void btAdauga_Click(object sender, EventArgs e)
        {
            try
            {
                using (var connection = _connection)
                {
                    ...
                    command.CommandText = "UPDATE Vehicul SET TimpulSosire = $TimpulSosire" +
                        " WHERE Id = $Id";
                    command.Parameters.AddWithValue("$Id", Convert.ToInt32(tbId.Text));
                    command.Parameters.AddWithValue("$TimpulSosire", tbTimpSosire.Text);
